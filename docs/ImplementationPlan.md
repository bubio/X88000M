...
X88000 (Manuke氏作 PC-8801エミュレータ, 元はLinux/GTK2版) を macOS に移植中。これまでに次が完了している。
...
3. **SDL3+ImGui フロントエンドのブートストラップ** (commit `01b950f`, `2d9d80f`, `3b10327`)
   — `src/sdl3/Sdl3FrontendMain.cpp` (約1100行) に集約。`x88core` 静的ライブラリと分離済み。

ただし SDL3版は機能が未充足で、特にサウンド (`PC88Opna.{h,cpp}` はタイマー部のみのスタブで FM/SSG 合成は未実装)、環境設定ダイアログ、ディスク/テープイメージ管理 UI、ドラッグ&ドロップ、設定永続化、フルスクリーンが未対応。元 X88000 相当の動作 (起動 → BASIC ROM → ゲームが遊べる + 音が出る) を SDL3版でも実現することがゴール。

最終マイルストーンは **SDL3版がレガシー GTK版と同等の操作性で動作し、YM2203 (FM 3ch + SSG 3ch) からの音が出る** こと。レガシー GTK版は当面 `X88000M_BUILD_LEGACY_GTK=ON` でビルド可能なまま残し、リファレンスとして併存させる。
...
- UI は元の GTK版と同等の構成を基本とし、わかりにくい箇所のみ ImGui ならではの形に調整。
...
| Phase | 内容 |
|---|---|
| **A** | SDL3 フロント基本機能 (音以外) — 設定/Env/イメージ管理/D&D/フルスクリーン |
| **B** | 音声出力経路 (Sdl3AudioOutput + Beep/PCG) |
| **C** | YM2203 自前実装 (FM 3ch + SSG 3ch) |
| **D** | デバッガ & プリンタの SDL3 移植 (X88000 機能パリティの完成) |

---

## Phase A — SDL3フロントエンドの機能パリティ仕上げ (音声以外)

ゴール: SDL3版だけで「ROMロード → ディスク挿入 → BASIC起動 → ゲーム操作」が GTK版と同じ感覚でできる。
...
- 元 GTK の IDM_BASICMODE_* / IDM_BASE_CLOCK_* は Settings → Environment ウィンドウに集約 (元はメニュー直下にズラッと並んでわかりにくい)。
- 元 GTK の Disk/Tape の挿入と管理が分離している箇所は、Media メニュー内で「Drive 1: ▸ (Empty / Manage...)」のようにサブメニュー化。
...
ゴール: X88000 の売りであるデバッガ機能と、プリンタエミュレーション一式を SDL3版にも移植し、レガシー GTK版と完全に同等の機能セットにする。Phase A〜C で本体動作と音が安定してから着手する (デバッガを早期に着手するとフロント基盤の変動に振り回されるため)。
...
すべて移植して GTK版との機能差をゼロにする。これが Phase D 完了 = SDL3版の機能パリティ達成。

**D 完了の検証:**
- SDL3版だけで X88000 GTK版でできた操作がすべて行える (機能差リストを作成して照合)。
- デバッガで Z80 のステップ実行・ブレーク・逆アセンブルが動作。
- 既知のテストROM/ゲームでブレークを張り、レジスタ追跡ができる。
- パラレルプリンタにテキスト出力 → プレビューに反映 → 用紙操作可能。
- レガシー GTK版を `X88000M_BUILD_LEGACY_GTK=ON` でビルドし、SDL3版と挙動を比較し差異がないことを確認。
...
- **Phase D 完了時:**
  - SDL3版だけで X88000 GTK版でできた操作がすべて行える。
  - デバッガで Z80 のステップ実行・ブレーク・逆アセンブルが動作。
  - パラレルプリンタにテキスト出力 → プレビューに反映 → 用紙操作可能。
  - レガシー GTK版 (`-DX88000M_BUILD_LEGACY_GTK=ON`) のビルドが引き続き通る。
