...
> このプランは当初立てた移植ロードマップを記録として残したものです。本文中の各フェーズは下記の通りすべて完了しており、v1.0.2 は「SDL3 版が旧 frontend と同等の操作性で動作し、YM2203 (FM 3ch + SSG 3ch) が fmgen 参照実装と A/B で区別が難しいレベルに近い音色で鳴る」段階に到達しました。残っているのは decap 由来 LUT 相当の精度が必要な領域のみ (詳細は `docs/SoundHardware.md` 末尾の「残課題 (v1.0.2 時点)」を参照)。
>
> **追記 (2026-05):** この文書は YM2608 追加前の計画記録です。現在の SDL3 版は設定で YM2203 / YM2608 native / YM2203 + Sound Board II を切り替え、YM2608 の FM6ch・リズムWAV・ADPCM初期実装に対応しています。本文中の「OPNAには対応しない」は当時のスコープ方針であり、現行仕様ではありません。
...
5. **FM 音質の二次改善 (v1.0.2)**: op output 14-bit 化、modulation `>> 1` + feedback `>> (10-FB)` の整合、envelope rate 一律 4x 低速化で fmgen 参照実装に近接 (詳細は `docs/SoundHardware.md` のバグ修正履歴 #6, #7, #11, #12)。
